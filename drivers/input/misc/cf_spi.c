/**
 * The platform spi device for chipsailing's fingerprint sensor.
 *
...
#define CF_SPI_BUS_NUM 8 // TODO:
#define CF_SPI_CS__NUM 0 // TODO:

#define MODULE_NAME "cf_spi"
...
static struct spi_board_info spi_desc __initdata = {
    .modalias        = "spidev",
    .bus_num         = CF_SPI_BUS_NUM,
    .chip_select     = CF_SPI_CS__NUM,
    .mode            = SPI_MODE_0,
    .platform_data   = NULL,
...
int cf_spi_clock_enable(bool on)
{
    int err = 0;
    xprintk(KERN_DEBUG, "%s(..) enter.\n", __FUNCTION__);
...
int __init cf_spi_platform_init(void)
{
    int err = 0;
    spi_ctl = spi_busnum_to_master(CF_SPI_BUS_NUM);
    ...
    //      return (-EBUSY);
    //    }
    err = cf_spi_clock_init();

    if (err) {
        return err;
    }

    cf_spi_clock_enable(true);
    xprintk(KERN_INFO, "chipsailing spi board info has been registered.\n");
    return err;
}

void __exit cf_spi_platform_exit(void)
{
    cf_spi_clock_enable(false);
    ...
    xprintk(KERN_INFO, "chipsailing spi board info released.\n");
}
