...
static int ilitek_short_test(int *short_data1, int *short_data2) {
	...
	for (i = 0; i < i2c.x_ch; i++) {
		if (abs(short_data1[i] - short_data2[i]) > i2c.ilitek_short_threshold) {
			printk("[TP_selftest]%s:%d err,short_test_result error short_data1[%d] = %d, short_data2[%d] = %d,i2c.ilitek_short_threshold = %d\n",
	...
	for(i = 0; i < test_32; i++) {
		...
		for(index = 2; index < newMaxSize; index += 2) {
			...
			if (((buf_recv[i * newMaxSize + index + 1] << 8) + buf_recv[i * newMaxSize + index]) < i2c.ilitek_open_threshold) {
				printk("[TP_selftest]%s:%d err,open_test_result error open_data[%d] = %d,i2c.ilitek_open_threshold = %d\n",
	...
	for(i = 0; i < test_32; i++) {
		...
		for(index = 2; index < newMaxSize; index += 2) {
			...
			if ( (allnode_data[j] < i2c.ilitek_full_raw_min_cap) ||(allnode_data[j] > i2c.ilitek_full_raw_max_cap) ) {
				printk("[TP_selftest]%s:%d err,allnode_test_result error allnode_data[%d] = %d, i2c.ilitek_full_raw_min_cap = %d i2c.ilitek_full_raw_max_cap = %d\n",
...
static int ilitek_change_report_rate_test(void)
{
 ...
 int ilitek_TP_self_test_runing(void)
{
	int i = 0;
	int j = 0;
	int ret = -1;
	...
	if (allnode_data) {
       ...
       {
            test_result = 1;     //pass the test
       } else 
       {
            test_result = 0;    //fail the test
       }
	return 0;
}
...
static ssize_t  Ilitek_touch_test_type_show(struct kobject *dev,
		struct kobj_attribute *attr, char *buf)
{
        int count;
        if(test_result == 1)
            count = sprintf(buf, "PASS\n");
         else count = sprintf(buf, "FAIL\n");

         printk("[TP_selftest] exit %s \n",__func__);
         return count;
}
...
static struct kobj_attribute Ilitek_touch_test_type = {
	.attr = {.name = "tp_capacitance_test_type", .mode = S_IRUGO|S_IWUSR},
	.show =  Ilitek_touch_test_type_show,
	.store =  Ilitek_touch_test_type_store,
};
#if 0
...
void ilitek_touch_node_init(void)
{
	...
	ret = sysfs_create_file(android_touch_kobj, &Ilitek_touch_test_type.attr);
	if (ret) {
