...
static int isCreatTP_info_node = 0;
//add for tp gesture node start

int ilitek_gesture_enable = 0;
...
static ssize_t gesture_store(struct device *dev, struct device_attribute *attr, const char *buff, size_t count)
{
   // char buf[80] ={0};
    ...
    {
        ilitek_gesture_enable = 1;
    } else {
        ilitek_gesture_enable = 0;
    }
...
{
     int count;
     count = sprintf(buf, "ilitek_gesture_enable =  %d\n", ilitek_gesture_enable);
     return count;
}
...
static void udp_receive(struct sk_buff  *skb)
{
	...
	ili_pid  = 100;//NETLINK_CREDS(skb)->pid;
	...
	{
		//printk("recv skb from user space uid:%u pid:%d seq:%d\n",uid,pid,seq);
		printk("data is :%s\n",(char *)data);
		udp_reply(ili_pid,seq,data,sizeof("Hello World!"));
...
#ifdef DEBUG_NETLINK
		if (debug_flag) {
			udp_reply(ili_pid,seq,buf,sizeof(buf));
		}
		if (buf[1] == 0x5F) {
			return 0;
		}
...
int ilitek_i2c_read_tp_info( void)
{
	...
	//add for tp info
	if(isCreatTP_info_node == 0)
       {   
	    memset(fw_version, 0, sizeof(fw_version));
	    sprintf(fw_version, "[FW]0x%x, [IC]ili2120", buf[2]);
	    init_tp_fm_info(0, fw_version, "ilitek");
           isCreatTP_info_node = 1;
       }else{
           memset(fw_version, 0, sizeof(fw_version));
	    sprintf(fw_version, "[FW]0x%x, [IC]ili2120", buf[2]);
...
void send_ilitek_TP_suspend_scnd_cmd(void)
{
	uint8_t cmd[2] = {0};
	int ret = 0;
	...
	printk("Enter ilitek_i2c_suspend 0x01 0x00\n");
	cmd[0] = 0x01;
	cmd[1] = 0x00;
...
//#ifdef GESTURE
	...
	*/
	...
	ret = ilitek_i2c_transfer(client, msgs_cmd, 1);
	if(ret < 0){
		printk("%s, 0x0A 0x01 set tp suspend err, ret %d\n", __func__, ret);
	}
...
#ifdef GESTURE
	system_resume_ilitek = 1;
        input_set_capability(i2c.input_dev, EV_KEY, KEY_WAKEUP);
