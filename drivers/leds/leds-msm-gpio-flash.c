...
int led_gpio_flash_probe(struct platform_device *pdev)
{
	...
	flash_led = devm_kzalloc(&pdev->dev, sizeof(struct led_gpio_flash_data),
				 GFP_KERNEL);
printk("zgy led_gpio_flash_probe 01\n");
	if (flash_led == NULL) {
...
printk("zgy led_gpio_flash_probe 02 rc = %d,temp_str = %s\n",rc,temp_str);
	if (!rc)
...
printk("zgy to register led dev. rc = %d\n",rc);
	if (rc) {
...
printk("zgy leds-msm-gpio-flash probe successfully!\n");
	return 0;
printk("zgyprobe successfully!\n");
error:
	if (IS_ERR(flash_led->pinctrl))
...
{
	int leds_flash;
	printk("zgy_init led_gpio_flash_init01\n");
...
{
    printk("zgy_exit led_gpio_flash_exit");
