...
static enum Tfa98xx_Error tfa9912_specific(Tfa98xx_handle_t handle) {
	...
	if (handles_local[handle].rev == 0x1a13) {
		/* -----  version 1.22 ----- */
		tfa98xx_write_register16(handle, 0x00, 0x1645); //POR=0x0645
		tfa98xx_write_register16(handle, 0x01, 0x828b); //POR=0x82ca
		tfa98xx_write_register16(handle, 0x02, 0x6dc8); //POR=0x6828
		...
		tfa98xx_write_register16(handle, 0x52, 0x7afc); //POR=0x7af4
	}
