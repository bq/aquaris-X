...
enum Tfa98xx_Error tfaRunSpeakerStartup(Tfa98xx_handle_t handle, int force, int profile)
{
	...
	printk("coldstart%s :", force? " (forced)":"");
	...
	if ( !force ) { // in case of force CF already runnning
		...
		if(handles_local[handle].slave_address > 10) {
			...
			printk("----- Profile (I2C) settings softDSP -----\n");
	...
	if (tfa_cf_enabled(handle)) {
		...
		if (tfa98xx_runtime_verbose) {
			if (TFA_GET_BF(handle, NOCLK))
				printk("Using internal clock\n");
			printk("Waiting for DSP system stable...\n");
		}
		...
		if (tries == CFSTABLE_TRIES) {
			if (tfa98xx_runtime_verbose) printk("Timed out\n");
			return Tfa98xx_Error_StateTimedOut;
		}  else
			if (tfa98xx_runtime_verbose) printk(" OK (tries=%d)\n", tries);
	}
