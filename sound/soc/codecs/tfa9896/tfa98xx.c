...
static enum tfa_error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int *vstep)
{
	...
	if(1){
		unsigned int ie_reg0 = 0,ie_reg4 = 0,ie_reg9 = 0;
		regmap_read(tfa98xx->regmap, TFA98XX_STATUSREG, &ie_reg0);
		regmap_read(tfa98xx->regmap, TFA98XX_I2SREG, &ie_reg4);
		regmap_read(tfa98xx->regmap, TFA98XX_SYS_CTRL, &ie_reg9);
		printk( "Restored interrupts: 0x00 = 0x%04x,0x04 = 0x%04x,0x09 = 0x%04x\n",ie_reg0,ie_reg4,ie_reg9);
	}
...
static void __tfa98xx_inputdev_check_register(struct tfa98xx *tfa98xx, bool unregister)
{
	...
	if (tfa98xx->input)
		dev_err(tfa98xx->codec->dev, "Input device already registered, skipping\n");
	else
	...
	if (otc->deferrable) {
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			...
			if (otc->rd_valid) {
				/* read cached value */
				*val = otc->rd_value;
				printk("Returning cached value of OTC: %llu\n", *val);
			} else {
...
static int tfa98xx_dbgfs_otc_set(void *data, u64 val)
{
	...
	if (otc->deferrable) {
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			...
			otc->wr_value = val;
			otc->triggered = true;
			printk("Deferring write to OTC (%d)\n", otc->wr_value);
			return 0;
		}
	}
...
static int tfa98xx_dbgfs_mtpex_set(void *data, u64 val)
{
	...
	if (mtpex->deferrable) {
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			...
			printk("Deferring write to MTPEX (%d)\n", mtpex->wr_value);
			return 0;
		}
	}
				     ...
				     loff_t *ppos)
{
	...
	printk("calib_done: %d - ret = %d - %s", calibrate_done, ret, str);
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (profile == tfa98xx_profile) {
		...
		if (ready) {
			...
			if (err) {
				...
				printk("Succesfully changed vstep index!\n");
			}
		}
	...
	printk("vstep:%d, (control value: %d) - profile %d\n", new_vstep,
								 value, profile);
	return (err == 0);
}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("selected container profile [%d]\n", prof_idx);
	...
	if (ready) {
		...
		if (err) {
			...
			printk("Changed to profile %d (vstep = %d)\n", prof_idx,
							tfa98xx_vsteps[0]);
		}
	}
...
{ 
	...
	list_for_each_entry(bprof, &profile_list, list) {
		if (0 == strncmp(bprof->basename, basename, len)) {
			...
			printk("added profile:samplerate = [%d:%d] for mixer profile: %s\n", profile, sr, bprof->basename);    
		}
	}
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	if (tfa98xx->set_mtp_cal == false) {
		printk("Calibration value (%d) set in mtp\n",
                        tfa98xx->cal_data);
	}
...
static void tfa98xx_unregister_dsp(struct tfa98xx *tfa98xx)
{
	...
	dev_err(&tfa98xx->i2c->dev, "Un-registered DSP instance with handle %d\n",
							tfa98xx->handle);
}
							...
							unsigned int mask)
{
	...
	if (reg & errs & ~mask) {
		...
		dev_err(tfa98xx->codec->dev, "status_reg events: 0x%x\n", reg);
	}
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
		printk("Skipping tfa_start (no FW: %d)\n", tfa98xx->dsp_fw_state);
		return;
	}
...
static void tfa98xx_interrupt(struct work_struct *work)
{
	...
	if (out1) {
		...
		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTNOCLK, out1) > 0) {
			...
			if(no_clk == 1) { 
				...
				printk("No clock detected. Resetting the I2CR to avoid pop on 72! \n");
		
				err = tfa98xx_tfa_start(tfa98xx, sw_profil, tfa98xx_vsteps);
				if (err != Tfa98xx_Error_Ok) {
					pr_err("Error loading i2c registers (tfa_start), err=%d\n", err);
				} else {
					printk("Setting i2c registers after I2CR succesfull\n");
				}
			}
	...
	if (out3) {
		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTLP0, out3) > 0) {
			...
			if (lp0 > 0) {
				printk("lowpower mode 0 detected\n");
			} else {
				printk("lowpower mode 0 not detected\n");
			}
		...
		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTLP1, out3) > 0) {
			...
			if (lp1 > 0) {
				printk("lowpower mode 1 detected\n");
			} else {
				printk("lowpower mode 1 not detected\n");
			}
						...
						struct snd_soc_dai *dai)
{
	...
	for (prof = 0; prof < nprof; prof++) {
		if (0 == strncmp(basename, tfaContProfileName(tfa98xx->handle, prof), len)) {   
			...
			if (!sr)
				dev_err(codec->dev, "Unable to identify supported sample rate\n");
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (no_start) {
		printk("no_start parameter set no tfa_start or tfa_stop, returning\n");
		return 0;
	}
...
static struct snd_soc_dai_driver tfa98xx_dai[] = {
	...
	//	.symmetric_channels = 1,   //delete it
...
static void __tfa98xx_irq(struct tfa98xx *tfa98xx)
{
	...
	dev_err(&tfa98xx->i2c->dev, "interrupt: 0x%04x (enabled: 0x%04x)\n", val, ie);
#ifdef DEBUG
	if (!(val & ie)) {
		unsigned int ireg;
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(tfa98xx->irq_gpio) &&
		...
		dev_err(&i2c->dev, "Skipping IRQ registration\n");
	...
	if (ret)
		dev_err(&i2c->dev, "error creating sysfs files\n");
